...
import {getAccessToken, getQAReport, getUser} from './zoom.js';
...
app.post('/', express.json(), async function webhookHandler(req, res) {
	...
	if (event === 'meeting.ended') {
		...
		logger.info(`Meeting ${meetingID} ended.  Generating Q&A report.`);
		...
		const report = await getQAReport(accessToken, meetingID);
		if (!!report === false) {
			logger.error(`Error getting Q&A report | ${report}`);
			return;
		}

		logger.info(`E-Mail:  ${email} | Q&A Report: ${JSON.stringify(report)}`);
	}
});
